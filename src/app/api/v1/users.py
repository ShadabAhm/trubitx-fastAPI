...
@router.get("/user/{username}/campaign_limits")
async def read_user_campaign_limits(
    request: Request, 
    username: str, 
    db: Annotated[AsyncSession, Depends(async_get_db)],
    current_user: Annotated[dict, Depends(get_current_user)]
) -> dict[str, Any]:
    """Get user's PR campaign limits"""
    from ...services.subscription_service import SubscriptionService
    from ...schemas.campaign_limits import CampaignLimitsResponse
    
    # Users can only check their own limits unless superuser
    if username != current_user["username"] and not current_user.get("is_superuser"):
        raise ForbiddenException()
    ...
    limits = await subscription_service.get_user_limits(db_user.id)
